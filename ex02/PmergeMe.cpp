...
void    PmergeMe::write_output_for_vec()
{
    ...
    std::cout << std::endl;
    std::cout << "Time to process a range of " << this->vec.size() << " elements with std::vector : " << (float)this->vec_time/CLOCKS_PER_SEC * 1000.0 << "ms" << std::endl;
}

void    PmergeMe::write_output_for_deq()
{
    std::cout << "Time to process a range of " << this->deq.size() << " elements with std::deque : " << (float)this->deq_time/CLOCKS_PER_SEC * 1000.0 << "ms" << std::endl;
}
